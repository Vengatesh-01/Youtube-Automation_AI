...
if not os.path.exists(PATH_BLENDER) and shutil.which("blender"):
    PATH_BLENDER = shutil.which("blender")

MODEL_CHARACTER = os.path.abspath("work/model/character.blend")
TEMPLATE_BASE = os.path.abspath("assets/character_v2.blend")
...
from cartoon_scripts.ollama_gen import generate_script
from cartoon_scripts.piper_tts import text_to_speech
from cartoon_scripts.rhubarb_sync import generate_lipsync
from cartoon_scripts.character_factory import ensure_character
from cartoon_scripts.comfy_gen import generate_content
from cartoon_scripts.youtube_upload import upload_video
...
def run_production_pipeline(topic):
    ...
    blender_script = os.path.abspath("cartoon_scripts/blender_render.py")
    cmd = [
