...
OUTPUT_PATH = os.path.abspath("outputs/facial_features_final_v2.png")
...
def setup_face_material(anchor):
    ...
    brow_color = hex_to_rgb("#5A341C") # Matched to hair
    lip_color = hex_to_rgb("#C97F7F")
    eyelash_color = (0, 0, 0, 1)
    ...
    # 2. Eyes
    eye_z = Az + 0.02
    eye_x_off = 0.032 # Slightly narrower
    
    def add_eye(base_color, side):
        ex = Ax + (eye_x_off * side)
        # Sclera (MUCH SMALLER)
        sclera = create_mask(ex, eye_z, 0.012, 0.018)
        res_s = mix_rgba(sclera, base_color, (1,1,1,1))
        # Iris
        iris_mask = create_mask(ex, eye_z, 0.008, 0.01)
        ...
        # Eyelashes (Very thin arc)
        lash = create_mask(ex, eye_z + 0.008, 0.012, 0.0015)
        res_l = mix_rgba(lash, res_h, eyelash_color)
        return res_l

    res2 = add_eye(res1, 1)
    res3 = add_eye(res2, -1)
    
    # 3. Curved Eyebrows (Lowered and Thinner)
    ...
    brow_r = create_curved_brow(Ax + 0.035, Az + 0.065) # Lowered from 0.085
